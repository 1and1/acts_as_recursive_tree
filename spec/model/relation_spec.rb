...
describe 'Relation' do
  ...
  context 'descendants' do
    ...
    it 'works with simple relation' do
      desc = @root.descendants(recursive_condition: Node.where(active: true))
    ...
    it 'works with joins relation' do
      desc = @root.descendants(recursive_condition: Node.joins(:node_info).where.not(node_infos: { status: 'bar' }))
      desc.all.each do |node|
  ...
  context 'ancestors' do
    ...
    it 'works with simple relation' do
      ancestors = @root.leaves.first.ancestors(recursive_condition: Node.where(active: false)).all.to_a
    ...
    it 'works with joins relation' do
      ancestors = @root.leaves.first.ancestors(recursive_condition: Node.joins(:node_info).where.not(node_infos: { status: 'foo' }))
