module ActsAsRecursiveTree
  module Query
    ...
    def related_items(clazz: nil, descendants: true, arel: false)
      related_recursive_items(self.id, clazz: clazz, descendants: descendants, arel: arel)
    end
    ...
    def related_recursive_items(id, clazz: nil, descendants: true, arel: false)
      item_table   = self.class.arel_table
      ...
      unless arel
        ...
        clazz.from(query.as(recursive_tree_config[:base_class].constantize.table_name).to_sql)
      else
        query
      end
    end
